...
Ce projet met en oeuvre différents types de relais TCP et HTTP. Notre objectif est d'insérer un ou plusieurs relais entre le client et le serveur.
On expérimentera ici 3 types de relais :
...
Le dossier se divisera donc en deux dossiers "Exercice1" et "Exercice2".

L'exercice 1 comporte la base d'un relai qui transmet simplement la requête du client au serveur et la réponse du serveur au client.
Voici la liste des fichiers de l'exercice 1 :
Client.py       : client qui envoie une requête au relai ("abc")
Relai.py        : relai qui joue le rôle d'intermédiaire entre le serveur et le client
Serveur.py      : serveur TCP/HTTP qui répond aux requêtes du relai

Voici la liste des fichiers de l'exercice 2 :
...
RelaiCenseur.py	: relai qui bloque l’accès à certains sites (interdit1, interdit2) et qui stocke dans interdit.log toute adresse client qui tente d'accéder à l'un de ces sites
