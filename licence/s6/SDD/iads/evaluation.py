...
def crossval_strat(X, Y, n_iterations, iteration):
    ...
    Pour chaque classe, on divise aléatoirement les indices en n_iterations parties
    """
    ...
    for cl in classes:
        ...
        n_cl = len(idx) // n_iterations
        #pour la dernière itération, on prend tous les indices restants
        if iteration == n_iterations - 1:
            end = len(idx)
        else:
            end = (iteration + 1) * n_cl
        start = iteration * n_cl
        idx_test_list.append(idx[start:end])
